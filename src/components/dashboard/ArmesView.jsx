        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(a => (
            <div key={a.id} className="rounded-2xl p-5 relative"
              ...
              {a.id_argent && <div className="text-[11.5px] flex items-center gap-1.5" style={{ color: '#808080' }}><span style={{ color: '#666' }}>ID arme :</span> {a.id_argent}</div>}
      ...
      <div className="w-full max-w-[440px] rounded-2xl p-6" style={{ background: '#1c1c1c', border: '1px solid rgba(255,255,255,0.08)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 rounded-xl px-4 py-3" style={{ background: '#121212', border: '1px solid rgba(255,255,255,0.06)' }}>
            <span className="text-[12px]" style={{ color: '#808080' }}>Encaissement au retour</span>
            <div className="text-[13px] font-semibold text-white mt-0.5">{(arme.prix_location || 0) + (parseFloat(form.caution) || 0)} $ (location + caution)</div>
          </div>
          <div className="col-span-2 flex justify-end gap-2 mt-1">
            <button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-[13px] font-medium" style={{ color: '#ccc', background: '#121212' }}>Annuler</button>
