...
export default function ObjetsView({ objets, categories, onAdd, onDelete, canAdd, canDelete }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-3 mb-6">
        <div className="flex items-center gap-2.5 rounded-full px-4 py-2.5 lg:w-[280px]"
          ...
          {filtered.map(o => (
            <div key={o.id} className="rounded-2xl p-5 relative"
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="text-[15px] font-bold text-white">{o.prix ? fmt(o.prix) : '—'}</div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div className="w-full max-w-[480px] rounded-2xl p-6" style={{ background: '#1c1c1c', border: '1px solid rgba(255,255,255,0.08)' }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-[17px] font-semibold text-white">Ajouter un objet</h3>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 flex justify-end gap-2 mt-1">
            <button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-[13px] font-medium" style={{ color: '#ccc', background: '#2A2A2A' }}>Annuler</button>
            <button type="submit" className="rounded-xl px-4 py-2.5 text-[13px] font-semibold text-white" style={{ background: '#FF6E40' }}>Ajouter</button>
          </div>
        </form>
      </div>
    </div>
